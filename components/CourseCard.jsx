...
const CourseCard = ({ card, addComment }) => {
  const { id, title, content, price, language, duration, location, comments, difficulty } = card;
  ...
  return (
    <div className='card'>
      ...
      {difficulty === 'beginner' ?
        <p className='level-style-beginner'>非常简单 <CheckCircleIcon /></p> :
      ...
      <div className='card-sub-container'>
        ...
        {!hasReview
          ? <AddComment addComment={addComment} onCommentAdded={() => setReview(true)} cardId={id} />
      ...
      <div className='button-container'>
        ...
        <button className={`button-style ${difficulty === 'beginner' ? 'beginner' : 'advanced'}`} onClick={() => setLike(like + 1)}>
          {difficulty === 'beginner' ? '现在就加入' : '还是再看看吧'}
        </button>
      </div>
    </div>
  )
